...
// Apenas Guide abre o menu — Start e Select ficam livres pro jogo
const MENU_BTNS = [16]
...
export default function PlayerPage() {
  ...
  // useLayoutEffect garante que o container está no DOM ANTES de carregar o EmulatorJS
  useLayoutEffect(() => {
    if (phase !== 'playing' || !game || !playInfo) return
    ...
    // Carrega o script — container já está no DOM garantido pelo useLayoutEffect
  ...
  // Poll gamepad — ignora primeiros 800ms (flush do B do RetroVision)
  ...
  if (phase === 'error') return (
    <div className="min-h-screen bg-black flex flex-col items-center justify-center gap-4">
      ...
      <button onClick={() => navigate(-1)} className="text-steam-accent hover:underline text-sm">← Voltar</button>
    </div>
  )
...
function GameMenu({ game, onClose, onBack, onFullscreen }) {
  ...
  // Poll próprio do menu — não interfere com o EmulatorJS
  useEffect(() => {
    const poll = () => {
      const gps = navigator.getGamepads?.() || []
      for (const gp of gps) {
